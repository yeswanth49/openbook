...
export function ConversationMetadata({ space, compact = false, className }: ConversationMetadataProps) {
  ...
  const lastActivityTime = formatDistanceToNow(new Date(metadata.lastActivity), { addSuffix: true });
  
  if (compact) {
    // Compact view for sidebar - optimized for hover animations
    return (
      <div className={cn("flex items-center gap-1.5 text-xs", className)}>
        {metadata.sentiment !== 'neutral' && SentimentIcon && (
  ...
  return (
    <div className={cn("flex flex-col space-y-2", className)}>
      <div className="flex flex-wrap gap-1.5">
        ...
        {metadata.sentiment !== 'neutral' && (
          <Badge variant="outline" className={cn(
            "text-xs",
            metadata.sentiment === 'positive' ? "bg-green-100 dark:bg-green-950 text-green-800 dark:text-green-300" : 
            metadata.sentiment === 'negative' ? "bg-red-100 dark:bg-red-950 text-red-800 dark:text-red-300" : ""
          )}>
      ...
      {metadata.summary && (
        <p className="text-sm text-muted-foreground line-clamp-2">{metadata.summary}</p>
      )}
    </div>
  );
} 