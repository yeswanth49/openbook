...
export const InteractiveStockChart = React.memo(({ title, data, stock_symbols, currency_symbols, interval, chart, news_results }: StockChartProps) => {
  ...
  const chartOptions = useMemo<EChartsOption>(() => {
    ...
    return {
      ...
      series: processedData.map((series, index) => {
        ...
        const yAxisIndex = hasMultipleCurrencies
          ? uniqueCurrencies.indexOf(series.currency)
          : 0;
      ...
      <div className="bg-card px-3 py-2 border-b border-border/40 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <div className="text-xs text-muted-foreground bg-muted/40 px-2 py-0.5 rounded-full flex items-center gap-1">
            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-primary/70">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-2">
          {processedData.map((series) => (
            <div key={series.label} className="p-2 rounded-md bg-card/40 border border-border/30">
              ...
              <div className={`text-xs ${series.priceChange >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
                {series.priceChange >= 0 ? '+' : ''}{series.percentChange}%
              </div>
            </div>
          ))}
        </div>
